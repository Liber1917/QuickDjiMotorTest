...
/**
  ...
  * @param data 发送信息的地址
  * @param StdID 发送信息的id
  * @param len 发送信息的长度(默认8字节)
  */
void Can_Tx(uint8_t* data,uint32_t StdID,uint8_t len)
{   
	CanTxMsg canTxMsg;

    canTxMsg.StdId = StdID; //id
    canTxMsg.ExtId = 0;
	canTxMsg.IDE = CAN_Id_Standard; //标准帧   
    canTxMsg.RTR = CAN_RTR_Data; //数据帧
    canTxMsg.DLC = len; //帧长度
	memcpy(canTxMsg.Data, data, len);

	CAN_Transmit(CAN1, &canTxMsg);
	delayUs(110);//防止CAN连发导致堵塞
}