...
void MultiImu::gyroAccUpdate()
{
	...
	for (int i = 0; i < VALID_IMU_NUM; i++)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			if ((validImu[i]->gyro.origin.data[j] == 0 ||
				 validImu[i]->gyro.origin.data[j] == 0xff || validImu[i]->gyro.origin.data[j] == -1 ||
				 validImu[i]->gyro.origin.data[j] == 32767 || validImu[i]->gyro.origin.data[j] == -32768) ||
				memcmp(lastGyroOriginData, thisGyroOriginData, sizeof(vec3int16)) == 0)
			{
				inValidDataCnt++;
			}

			if ((validImu[i]->acc.origin.data[j] == 0 ||
				 validImu[i]->acc.origin.data[j] == 0xff || validImu[i]->acc.origin.data[j] == -1 ||
				 validImu[i]->acc.origin.data[j] == 32767 || validImu[i]->acc.origin.data[j] == -32768) ||
				memcmp(lastAccOriginData, thisAccOriginData, sizeof(vec3int16)) == 0)
			{
				inValidDataCnt++;
			}
		}
		if (inValidDataCnt >= 3 && currentImuShouldUsed < VALID_IMU_NUM - 1 && currentImuShouldUsed == i)
		{
			currentImuShouldUsed++;
			//			validImu[i]->init();
			//			extern TickType_t PreviousWakeTime;
			//			PreviousWakeTime = xTaskGetTickCount();
		}

		//		//todo test whether one imu only update is premitted
		//		if (currentImuUsed != i)
		//			break;
	}
	...
	memcpy(&sixCaliFector, &validImu[currentImuUsed]->sixCaliFector, sizeof(sixCaliFector));
	memcpy(&sixCaliOffset, &validImu[currentImuUsed]->sixCaliOffset, sizeof(sixCaliOffset));
}