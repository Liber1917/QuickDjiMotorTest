...
void Power::powerOverFlowCal(void)
{
	...
	if(!ctrlCnt++ && judgement.powerHeatData.chassisPowerBuffer <= 0)//缓冲能量使用到限制以下，增大限制强度
	{
		overFlowKp -= 0.03f;
...
void Power::myPowerCanTx(void)
{
	...
	{
		chassisPower	 	 = judgement.powerHeatData.chassisPower;//底盘功率
		chassisPowerBuffer 	 = LIMIT(judgement.powerHeatData.chassisPowerBuffer,0,300);//底盘缓冲能量,限幅，防止充不进去电?
		max_chassis_power 	 = LIMIT(judgement.gameRobotStatus.chassisPowerLimit,50,120);//最大功率，限幅，防止裁判系统乱发数导致通道超载
